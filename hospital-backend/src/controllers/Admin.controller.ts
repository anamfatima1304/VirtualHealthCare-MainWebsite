...
export class AppointmentController {
  ...
  // ── FIXED: use Date.UTC() so boundaries are always UTC midnight regardless
  //           of server timezone. appointmentDate is stored as UTC ISO strings
  //           so the query range must also be in UTC.
  async getAppointments(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (date) {
        ...
        // ✅ FIX: Date.UTC ensures boundaries are always UTC midnight,
        //         matching how appointmentDate is stored (UTC ISO string).
        //         The old new Date(year, month-1, day) used local time which
        //         is fragile and was the root cause of slot mismatch.
